...
void UserAuthDatabaseManager::init() {
    QSqlQuery q(db);
    q.exec("CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY AUTOINCREMENT, nickname TEXT UNIQUE, password TEXT, email TEXT UNIQUE, avatar TEXT, interests TEXT)");

    q.exec("CREATE TABLE IF NOT EXISTS friends ( user_id INTEGER, friend_id INTEGER, UNIQUE(user_id, friend_id))");
